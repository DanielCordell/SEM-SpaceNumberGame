...
public class Starfield : MonoBehaviour
{
	...
	void Update()
	{
		for (int i = 0; i < stars.Length; i++) 
		{
			...
			if (pos.x < (cameraTransform.position.x - xOffset)) { pos.x += starFieldWidth; Debug.Log("a"); }
			else if (pos.x > (cameraTransform.position.x + xOffset)) { pos.x -= starFieldWidth; Debug.Log("b"); }
			if (pos.y < (cameraTransform.position.y - yOffset)) { pos.y += starFieldHeight; Debug.Log("c"); }
			else if (pos.y > (cameraTransform.position.y + yOffset)) { pos.y -= starFieldHeight; Debug.Log("d"); }
